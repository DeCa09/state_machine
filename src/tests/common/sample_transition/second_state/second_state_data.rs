use crate::state_machine::state::StateData;

#[derive(Debug, Clone, Default, PartialEq, PartialOrd, Hash, Eq, Ord)]
pub struct SecondStateData {
    state_data: String,
}

impl SecondStateData {
    pub fn new(state_data: String) -> Self {
        Self { state_data }
    }
}

impl StateData for SecondStateData {
    type UpdateType = SecondStateDataUpdater;
    fn get_state(&self) -> &Self {
        self
    }

    fn update_state(&mut self, updates: Self::UpdateType) {
        if let Some(value) = updates.state_data {
            self.state_data = value;
        }
    }
}

#[derive(Debug, Clone, PartialEq, PartialOrd, Hash, Eq, Ord)]
pub struct SecondStateDataUpdater {
    pub state_data: Option<String>,
}

pub struct SecondStateDataUpdaterBuilder {
    state_data: Option<String>,
}

impl SecondStateDataUpdaterBuilder {
    pub fn new() -> Self {
        Self { state_data: None }
    }

    pub fn state_data(mut self, state_data: String) -> Self {
        self.state_data = Some(state_data);
        self
    }

    pub fn build(self) -> SecondStateDataUpdater {
        SecondStateDataUpdater {
            state_data: self.state_data,
        }
    }
}

impl Default for SecondStateDataUpdaterBuilder {
    fn default() -> Self {
        Self::new()
    }
}
